...
def run_events(args, results_file):
        ...
        def run_single_event(event_number):
                ...
                logging.info('TRENTO_VAH started at %s ', start_h)

                h = run_cmd('./TRENTO_VAH')

                end_h = datetime.datetime.now()
                logging.info('TRENTO_VAH finished in %s ', end_h - start_h)
                ...
                if os.stat("output/surface.dat").st_size == 0:
                        raise StopEvent('empty surface')
                else:
                        subprocess.call("mv output/surface.dat input/surface.dat", shell=True)
...
def main():
        ...
        common="/home/ac.liyanage/vah_run_events/common"
        ...
        with open(args.results, filemode + 'b') as results_file, tempfile.TemporaryDirectory( prefix='jetscape-', dir=args.tmpdir) as workdir:
                ...
                os.mkdir("output/")             # VAH writes freezeout surface to output/surface.dat
                ...
                os.system('cp -R {:s}/parameters parameters'.format(tablesdir))

                # copy tables
                os.system('cp -R {:s}/PDG PDG'.format(common))
                os.system('cp -R {:s}/tables tables'.format(common))
                os.system('cp -R {:s}/deltaf_coefficients deltaf_coefficients'.format(common))
                os.system('cp -R {:s}/smash_input smash_input'.format(common))

                # copy executables
                os.system('cp {:s}/TRENTO_VAH TRENTO_VAH'.format(common))
                os.system('cp {:s}/SAMPLER_AFTERBURNER SAMPLER_AFTERBURNER'.format(common))
