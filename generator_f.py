...
def gen_random_sample(H, persis_info, gen_specs, _):
    ...
    param = persis_info['rand_stream'].uniform(lower, upper, (int(batch_size/2), num))
    out['x'] = np.concatenate((param, param), axis=0)
 
    # out['x'] = persis_info['rand_stream'].uniform(lower, upper, (batch_size, num))
    
    new_file = open("parameter.txt",mode="a+",encoding="utf-8")
    new_file.writelines(str(out['x']))
